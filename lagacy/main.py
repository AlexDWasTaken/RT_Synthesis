...
from process import load_checkpoints, headpose_pred_to_degree, keypoint_transformation 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--always_update_source", default=False, action="store_true")
    ...
    if args.no_pitch: print("Ignoring pitch while generating.")
    ...
    with torch.no_grad():
        ...
        while remote_ret and local_ret:
            ...
            if he_source == None or cached_coords != remote_cutout['coords'] or sharp_rotation(he_driving, he_source) or force_update or next(update_manager):
                print("################Changing source.##################")
                #############Test Code##############
                with open('log on source changing.txt', 'a') as f:
                    original_std_out = sys.stdout
                    sys.stdout = f
                    print("Frame", frame)
                    if he_source == None: 
                        print("HESource")
                    elif cached_coords != remote_cutout['coords']:
                        print("Detector")
                    elif sharp_rotation(he_driving, he_source):
                        print("Sharp rotation")
                    elif force_update:
                        print("Force update")
                    else:
                ...
                he_driving_initial = he_estimator(source) #add
                kp_driving_initial = keypoint_transformation(kp_canonical, he_driving_initial, estimate_jacobian)
                time_end = default_timer()
                
                print("Time elapsed in changing source: ", time_end - time_start)
                with open('log on source changing.txt', 'a') as f:
                    original_std_out = sys.stdout
                    sys.stdout = f
                    print("Time elapsed in changing source: ", time_end - time_start)
            ...
            kp_canonical = kp_detector(driving_frame) #add
            ...
            x, y, l = remote_face_detector.face_frame
        ...
        print("Saving...")
        imageio.mimsave(args.result, [img_as_ubyte((cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))) for frame in prediction], fps = fps)
